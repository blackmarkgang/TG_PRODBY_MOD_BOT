...
async def notify_application_decision(application: Application) -> dict[str, bool | str | None]:
    ...
    try:
        if application.status == "approved":
            ...
            text = "Ваша заявка в сообщество Prod.by одобрена."
            if application.admin_comment:
                text += f"\n\nКомментарий администрации:\n{application.admin_comment}"
            if invite_url:
                text += "\n\nСсылка действует 7 дней и рассчитана на одно вступление."
                ...
                text += "\n\nСсылка на вход пока недоступна. Администрация должна проверить права бота."
                keyboard = None
        else:
            text = "Ваша заявка в сообщество Prod.by отклонена."
            if application.admin_comment:
                text += f"\n\nКомментарий администрации:\n{application.admin_comment}"
            keyboard = None
